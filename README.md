[index.html.html](https://github.com/user-attachments/files/25539746/index.html.html)
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
...
<div class="modal-overlay" id="apiModal">
  <div class="modal-box">
    ...
    <div class="modal-err" id="modalErr">⚠️ Invalid key format. It should start with <code>sk-ant-</code></div>
    <div class="modal-hint">🔒 Stays in your browser only &nbsp;·&nbsp; Get yours at <a href="https://console.anthropic.com/settings/keys" target="_blank" style="color:var(--gold)">console.anthropic.com</a></div>
    <button class="modal-btn" onclick="saveKey()">✅ Start Generating Test Cases</button>
  </div>
</div>

<!-- KEY BAR -->
<div class="key-bar" id="keyBar">
  <span>🔑 API Key: <span id="keyPreview"></span></span>
...
<div class="product-section">
  ...
  <div class="ps-sub">Choose a product or group — modules will update automatically</div>

  <!-- Credit Life -->
  <div class="cat-section">
    <div class="cat-label cat-credit">Credit Life Products</div>
    <div class="chips-row">
      <div class="prod-chip" data-key="GSS" data-cat="credit" data-name="Group Smart Select" data-ico="🛡️" onclick="pickProd(this)">
        <span class="chip-ico">🛡️</span> Group Smart Select <span class="chip-ck">✓</span>
      </div>
      <div class="prod-chip" data-key="GSM" data-cat="credit" data-name="Group Smart Supreme" data-ico="👑" onclick="pickProd(this)">
        <span class="chip-ico">👑</span> Group Smart Supreme <span class="chip-ck">✓</span>
      </div>
      <div class="prod-chip" data-key="GAS" data-cat="credit" data-name="ABSLI Group Assured Shield" data-ico="🔰" onclick="pickProd(this)">
        <span class="chip-ico">🔰</span> ABSLI Group Assured Shield <span class="chip-ck">✓</span>
      </div>
      <div class="prod-chip" data-key="GAAP" data-cat="credit" data-name="Group Asset Assured Plan" data-ico="📋" onclick="pickProd(this)">
        <span class="chip-ico">📋</span> Group Asset Assured Plan <span class="chip-ck">✓</span>
      </div>
      <div class="prod-chip" data-key="GBY" data-cat="credit" data-name="ABSLI Group Bima Yojana" data-ico="🏦" onclick="pickProd(this)">
        <span class="chip-ico">🏦</span> ABSLI Group Bima Yojana <span class="chip-ck">✓</span>
      </div>
      <div class="prod-chip" data-key="ALL_CREDIT" data-cat="credit" data-name="All Credit Life Products" data-ico="🔵" onclick="pickProd(this)">
        <span class="chip-ico">🔵</span> All Credit Life Products <span class="chip-ck">✓</span>
      </div>
    </div>
  </div>
  ...
  <div class="cat-section">
    ...
    <div class="chips-row">
      <div class="prod-chip" data-key="GPS" data-cat="term" data-name="Group Protection Solution" data-ico="🌿" onclick="pickProd(this)">
        <span class="chip-ico">🌿</span> Group Protection Solution <span class="chip-ck">✓</span>
      </div>
      <div class="prod-chip" data-key="ALL_TERM" data-cat="term" data-name="All Term Life Products" data-ico="🟢" onclick="pickProd(this)">
        <span class="chip-ico">🟢</span> All Term Life Products <span class="chip-ck">✓</span>
      </div>
    </div>
  </div>
  ...
  <div class="cat-section" style="margin-bottom:0;">
    ...
    <div class="chips-row">
      <div class="prod-chip" data-key="ALL" data-cat="both" data-name="All Products (Credit Life + Term Life)" data-ico="⭐" onclick="pickProd(this)">
        <span class="chip-ico">⭐</span> All Products — Credit Life + Term Life <span class="chip-ck">✓</span>
      </div>
    </div>
  </div>
</div>
...
<div class="main-layout">
  ...
  <div class="left-panel">
    <div class="lp-block">
      ...
      <div class="sel-strip strip-credit" id="selStrip">
        <div class="ss-ico" id="selIco">—</div>
        <div>
  ...
  <div class="right-panel" id="rightPanel">
    <div class="empty-state">
      ...
      <div class="es-sub">Select a product, choose module & sub-module, paste your SRS — AI builds your complete TCD instantly.</div>
      <div class="steps">
        <div class="step"><div class="step-n">1</div><div class="step-t"><strong>Select product</strong> — individual or full category group</div></div>
        ...
        <div class="step"><div class="step-n">3</div><div class="step-t"><strong>Paste SRS</strong> and click Generate → export to Excel</div></div>
      </div>
    </div>
  </div>
</div>
...
<script>
// ─── PRODUCT → MODULE → SUBMODULE DATA (from Excel) ───
const PROD_DATA = {
  GPS: {
    name:"Group Protection Solution", cat:"term", ico:"🌿",
    modules:{
      "Home":["Home"],
  ...
  GSS: {
    name:"Group Smart Select", cat:"credit", ico:"🛡️",
    modules:{
      "Home":["Home"],
      "Masters":["Look Up","Sub Payout","Bank Branch","Address Structure","Pin Code Upload","Pin Code Defect Data"],
      "My Account":["Change Password","Change Product"],
      "Configuration":["Email Template","Endorsement Automation Verification","Insurer Organization Hierarchy","Insurer Organization Structure","Global Components","Product Setup","Channel Management","User Management","Deviation"],
      "Client Master":["Client Organization","Master Policy Freelook Factor Mapping","Master Policy Amortization Schedule","Advance Deposit Receipt","Master Policy Appendix Mapping","Master Policy Underwriting Mapping","Policy Account Statement","Questionnaire","Master Policy – Maker","Master Policy – Checker","Master Policy – View","Deposit Transfer Maker","Deposit Transfer Checker","Deposit Transfer View","Bank Statement Upload","Advance Deposit Receipt Mapping","Advance Deposit Receipt Defect Data","Bank Master","Deposit Receipt Cancellation Maker","Deposit Receipt Cancellation Checker","Bulk Receipt Upload","MPH Account Details"],
  ...
  GSM: {
    name:"Group Smart Supreme", cat:"credit", ico:"👑",
    modules:{
  ...
  GAS: {
    name:"ABSLI Group Assured Shield", cat:"credit", ico:"🔰",
    modules:{
...
const COMBO_KEYS = {
  ALL_CREDIT: { name:"All Credit Life Products", cat:"credit", ico:"🔵", keys:["GSS","GSM","GAS","GAAP","GBY"] },
  ALL_TERM:   { name:"All Term Life Products",   cat:"term",   ico:"🟢", keys:["GPS"] },
  ALL:        { name:"All Products (Credit Life + Term Life)", cat:"both", ico:"⭐", keys:["GPS","GSS","GSM","GAS","GAAP","GBY"] }
};
...
function loadModuleDropdown(modules) {
  ...
  document.getElementById('subModuleSelect').innerHTML = '<option value="">— Select Sub-Module —</option>';
}
...
function loadSubModules() {
  ...
  if (!mod || !activeProd) { sub.innerHTML = '<option value="">— Select Sub-Module —</option>'; return; }
...
async function generate() {
  if (!activeProd) { toast('⚠️ Please select a product first'); return; }
  const srs = document.getElementById('srsInput').value.trim();
  if (!srs) { toast('⚠️ Please paste your SRS or User Story'); return; }
...
- Return ONLY valid JSON array — no text outside
...
Schema: [{"sno":"1","tc_id":"${abbr}TC001","created_by":"muthukrishnan","product_name":"${activeProd.name}","module":"${fullModule}","functionality":"...","tc_type":"Positive","test_scenario":"...","test_case_description":"Step 1: ...\nStep 2: ...","expected_result":"...","actual_result":"As Expected","status":"Pass","defect":"N/A","tested_by":"muthukrishnan","tested_date":"${date}","comments":"N/A"}]`;
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages",{
      method:"POST", headers:{"Content-Type":"application/json","anthropic-version":"2023-06-01","x-api-key": window._apiKey},
      body:JSON.stringify({
    ...
    toast(`✅ ${generatedTCs.length} test cases generated!`);
  } catch(e) {
    document.getElementById('rightPanel').innerHTML = `<div class="err-box"><strong>⚠️ Error</strong><br/><br/><code>${e.message}</code></div>`;
  }
...
function renderTable(tcs, fullModule, cr, date) {
  ...
  document.getElementById('rightPanel').innerHTML=`
    <div class="res-bar">
      ...
      <div class="act-row">
        <button class="act-btn act-primary" onclick="exportExcel()">📥 Excel</button>
        <button class="act-btn" onclick="exportCSV()">📄 CSV</button>
        <button class="act-btn" onclick="copyTSV()">📋 Copy</button>
      </div>
    </div>
    <div class="stats">
...
function copyTSV(){
  if(!generatedTCs.length)return;
  navigator.clipboard.writeText([HDR,...rows()].map(r=>r.map(c=>String(c||'').replace(/\n/g,' ')).join('\t')).join('\n')).then(()=>toast('Copied! Paste into Excel ✓'));
}

// ─── UI HELPERS ───
function showLoading(){
  const msgs=['📖 Reading requirements...','🔍 Identifying test scenarios...','✍️ Writing Positive cases...','⚠️ Writing Negative cases...','📐 Adding Boundary & Exception...','📋 Formatting TCD...','✅ Almost ready...'];
  let i=0;
...
async function handleFileUpload(input) {
  ...
  try {
    ...
    if (text && text.trim()) {
      ...
      updCount();
      toast(`✅ ${file.name} loaded (${text.length.toLocaleString()} chars)`);
    } else {
      toast('⚠️ Could not extract text from file');
    }
  } catch(e) {
    toast('⚠️ File read error: ' + e.message);
  }
...
// ─── API KEY MODAL ───
window._apiKey = sessionStorage.getItem('absli_key') || '';
...
function saveKey() {
  ...
  window._apiKey = val;
  sessionStorage.setItem('absli_key', val);
  document.getElementById('apiModal').style.display='none';
  document.getElementById('keyBar').style.display='flex';
  document.getElementById('keyPreview').textContent = val.substring(0,18)+'••••••••';
  toast('🔑 API Key saved — ready to generate!');
}

function changeKey() {
  window._apiKey = '';
  sessionStorage.removeItem('absli_key');
...
if (!window._apiKey) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('apiModal').style.display='none';
    document.getElementById('keyBar').style.display='flex';
    document.getElementById('keyPreview').textContent = window._apiKey.substring(0,18)+'••••••••';
  });
}
